...
class TestConnectionManager:
    ...
    async def test_get_superuser_uri_production_mode(
        self, mock_api_client, mock_core_v1, mock_secret_data
    ):
        ...
        result = await ConnectionManager.get_superuser_uri(
    ...
    @patch("userinit.connections.client.CoreV1Api")
    @patch("userinit.connections.ApiClient")
    async def test_get_superuser_uri_dev_mode(
        self, mock_api_client, mock_core_v1, mock_secret_data
    ):
    ...
    async def test_get_superuser_uri_missing_uri(self, mock_api_client, mock_core_v1):
        ...
        with pytest.raises(kopf.TemporaryError, match="Could not parse connection uri"):
            await ConnectionManager.get_superuser_uri(
                "test-ns", "test-cluster", "postgres"
            )
    ...
    async def test_open_cluster_connection_success(self, mock_connect, mock_get_uri):
        ...
        result = await ConnectionManager.open_cluster_connection(
            "test-ns", "test-cluster", "postgres"
        )
